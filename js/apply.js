  ...
  try {
    ...
    if (houseTutorSelect && houseTutors) {
      houseTutors.forEach(tutor => {
        ...
        option.textContent = `${tutor.name} (${tutor.floor === 'Ground' ? 'Ground Floor' : `Floor ${tutor.floor}`})`;
...
document.getElementById('applicationForm').addEventListener('submit', async (e) => {
  ...
  if (!title || !description || !houseTutorId || !asstProvostId || !provostId || !treasurerId || !vcId || !file) {
    alert('Please fill all fields and select all officers');
    return;
  }
  ...
  try {
    ...
    // Create approval steps (fixed order)
    const approvals = [
    ...
    // Notify first officer (House Tutor)
    ...
    await sendNewFileNotification(fileData, houseTutorId);

    alert('✅ File submitted successfully! Email notification sent to House Tutor.');
